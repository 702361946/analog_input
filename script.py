...
global_value = {
    'step': 0,  # 当前步数
    'exevute': None,  # 存储转义字典
    'Developer Mode': False,  # 开发者模式
}
global_execute = {}
global_user_value = {}  # 全局变量
"""
name: value
"""


def get_scripts_file_names() -> list:
    # 存储找到的文件名
    filename_list = []
    # 遍历scripts目录
...
def parse_script(a: str, _split: str = ';', _x_h: float = 1, _x_w: float = 1) -> bool | dict:
    ...
    def lambda_value_translate(_key: str):
        """
        "!"转化的lambda实例
        """
        if _key not in global_user_value.keys():
            global_user_value[_key] = None  # 创建临时值(避免引发KeyError)

        return lambda: global_user_value[_key]
    ...
    try:
        ...
        match re['method']:
            case 'str':
                ...
                for i in message:
                    if i[0] == '!':
                        t.append(lambda_value_translate(i[1: len(i)]))
                        _t = True
                    else:
                        t.append(i)
                ...
                global_user_value[name] = message
                return True
            ...
            case 'list':
                ...
                for i in _list:
                    if i[0] == '!':
                        t.append(global_user_value[i[1: len(i)]])
                    else:
                        t.append(i)
                _list = t

                global_user_value[name] = _list
                return True
            ...
            case 'dict':
                ...
                for i in _dict.keys():
                    if _dict[i][0] == '!':
                        _dict[i] = global_user_value[_dict[i][1: len(_dict[i])]]

                global_user_value[name] = _dict
                return True
            ...
            case 'input':
                ...
                def __temp(_name: str, _message: str):
                    global_user_value[_name] = input(_message)
            ...
            case 'key':
                ...
                if key_one:
                    key = Key.Return._int(key)
                else:
                    key = Key.Return._int_strs(key)
...
def run(execute: dict, get_execute: bool = False) -> bool:
    ...
    input('按下回车开始执行')
    global_value['step'] = 0
    while global_value['step'] < execute['step']:
        i = execute[f'{global_value["step"]}']
        global_value['step'] += 1  # 避免goto重置指针错误
...
if __name__ == '__main__':
    ...
    # global_user_value['fff'] = 'eeeee'
    while True:
        script_names = get_scripts_file_names()
        for name_i in range(len(script_names)):
            print(f'{name_i} : {script_names[name_i]}')

        # global_value['Developer Mode'] = True  # 此行上线时应当注释掉
        if global_value['Developer Mode'] is True:
            user_input = '0'
        else:
